function [gbest,gbestval,hist_v,hist_p,FES]= PID_DMS_PSO_func(fhd,Dimension,group_num,Particle_Number,iter_max,VRmin,VRmax,varargin)
    ...
    for i = 1:threshold
        ...
        for k=1:ps
            ...
            vel(k,:)=(vel(k,:)<(-mv(k,:))).*(-mv(k,:))+(vel(k,:)>=(-mv(k,:))).*vel(k,:);
            pro(k,:)=alpha*pro(k,:) + (aa - last_aa(k,:));
            last_aa(k,:) = aa;
            pos1=pos(k,:)+ki*vel(k,:)+kp*pro(k,:); 
            keep_d=rand(1,D)<0.5;
