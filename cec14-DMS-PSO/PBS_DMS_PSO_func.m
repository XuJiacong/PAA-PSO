function [gbest,gbestval,hist_v,hist_p,FES]= PBS_DMS_PSO_func(fhd,Dimension,group_num,Particle_Number,iter_max,VRmin,VRmax,varargin)
    ...
    for i = 1:threshold
        ...
        for k=1:ps
            ...
            vel(k,:)=(vel(k,:)<(-mv(k,:))).*(-mv(k,:))+(vel(k,:)>=(-mv(k,:))).*vel(k,:);
            der(k,:)=alpha*der(k,:)+(gbestrep(k,:)-last_gbest(k,:));
