function [gbest,gbestval,hist_v,hist_p,FES]= PID_DMS_PSO_funcM(fhd,Dimension,group_num,Particle_Number,iter_max,VRmin,VRmax,varargin)
    ...
    [~,bbb]=sort(gbestval);
    gbestrep = repmat(gbest(bbb(1)),ps,1);
    ...
    for i = 1:threshold
        ...
        for k=1:ps
            ...
            vel(k,:)=(vel(k,:)<(-mv(k,:))).*(-mv(k,:))+(vel(k,:)>=(-mv(k,:))).*vel(k,:);
            der(k,:)=alpha*der(k,:)+gamma(i)*(pbest(k,:)-last_pbest(k,:))+(1-gamma(i))*(gbestrep(k,:)-last_gbest(k,:));
            last_gbest(k,:) = gbestrep(k,:);
            last_pbest(k,:) = pbest(k,:);
            pos1=pos(k,:)+vel(k,:)+kd*der(k,:); 
            keep_d=rand(1,D)<0.5;
