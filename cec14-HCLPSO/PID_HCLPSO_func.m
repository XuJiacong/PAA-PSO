function [gbest,gbestval,hist_v,hist_p,FES]= PID_HCLPSO_func(fhd,dimension,num_particle,max_iters,VRmin,VRmax,varargin) 
...
num_g1=15;
num_g2=ps-num_g1;
last_delta_g1 = zeros(num_g1,D);
last_delta_g2 = zeros(num_g2,D);
...
%         der(1:num_g1,:) = alpha*der(1:num_g1,:) + (delta_g1 - last_delta_g1);
%         last_delta_g1 = delta_g1;
        vel_g1=((vel_g1<Vmin).*Vmin)+((vel_g1>Vmax).*Vmax)+(((vel_g1<Vmax)&(vel_g1>Vmin)).*vel_g1);
        pos_g1=pos(1:num_g1,:)+vel_g1;%+kp*der(1:num_g1,:);
       ...
       % group 2 (Only apply in group 2)
        ...
        der(num_g1+1:end,:) = alpha*der(num_g1+1:end,:) + (delta_g2 - last_delta_g2);
        last_delta_g2 = delta_g2;
        pos_g2=pos(num_g1+1:end,:)+ki*vel_g2+kp*der(num_g1+1:end,:);
