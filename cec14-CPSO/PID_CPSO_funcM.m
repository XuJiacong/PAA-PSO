function [gbest,gbestval,hist_v,hist_p,fitcount]= PID_CPSO_funcM(fhd,Dimension,Particle_Number,iter_max, VRmin,VRmax,varargin)
    % CPSO-H6
    gamma = (0.9 - (1:double(iter_max)) .* (0.8./double(iter_max)));
    kd = 0.05;
    alpha = 0.9;
...
%% Main Loop
    ...
    for i = 2:me
        ...
        for k = 1:ps
            ...
            % Update der vector
            der(k,:) = alpha*der(k,:) + gamma(i)*(pbest(k,:)-last_pbest(k,:))+(1-gamma(i))*(gbestrep(k,:) - last_gbestrep(k,:));
