function [gbest,gbestval,hist_v,hist_p,fitcount]= PID_CPSO_func(fhd,Dimension,Particle_Number,iter_max, VRmin,VRmax,varargin)
    % CPSO-H6
    kp = 0.08;
    ki = 0.96;
    alpha = 0.9;
    ...
    for i = 2:me
        for j = 1:groupnum
            ...
            for k = 1:ps
                ...
                % Update Pro Vector
                pro1(k,group_d(j,:)) = alpha*pro1(k,group_d(j,:)) + (aa1(k,group_d(j,:)) - last_aa1(k,group_d(j,:)));
                last_aa1(k,group_d(j,:)) = aa1(k,group_d(j,:));
                ...
                % Update Position
                pos1(k,group_d(j,:)) = pos1(k,group_d(j,:)) + ki*vel1(k,group_d(j,:)) + kp*pro1(k,group_d(j,:));
        ...
        for k = 1:ps
            ...
            % Update pro vector
            pro(k,:) = alpha*pro(k,:) + (aa(k,:) - last_aa(k,:));
            last_aa(k,:) = aa(k,:);
            % Update Velocity Vector
