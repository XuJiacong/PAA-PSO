function [gbest,gbestval,hist_v,hist_p,fitcount]= PID_CLPSO_funcM(fhd,Dimension,Particle_Number,Max_Gen,VRmin,VRmax,varargin)
    gamma = (0.9 - (1:double(Max_Gen)) .* (0.8./double(Max_Gen)));%0.2.*ones(Max_iters,1);
    alpha = 0.9;
    kp = 0.05;
    ki = 1.00;
    kd = 0.0;
    ...
    for i = 2 : me
        for k = 1 : ps
            ...
            der(k, :) = alpha.*der(k, :) + (gamma(i)) .* (pbest(k, :)-2.*pbest_last(k, :)+pbest_last2(k, :)) + (1-gamma(i)) .* (gbest-2.*gbest_last+gbest_last2);
            pbest_last2(k, :) = pbest_last(k, :);
            gbest_last2 = gbest_last;
            
            pro(k, :) = alpha.*pro(k, :) + (gamma(i)) .* (pbest(k, :)-pbest_last(k, :)) + (1-gamma(i)) .* (gbest-gbest_last);
            pbest_last(k, :) = pbest(k, :);
            gbest_last = gbest;
