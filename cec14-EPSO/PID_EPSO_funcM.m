function [gbest,gbestval,hist_v,hist_p,FES]= PID_EPSO_funcM(fhd,Dimension,num_particle,max_iteration,VRmin,VRmax,varargin)
...
%             der(i,:)= alpha*der(i,:) + gamma(k)*(pbest(i,:)-last_pbest(i,:))+(1-gamma(k))*(gbestrep(i,:)-last_gbest(i,:));
%             last_gbest(i,:) = gbestrep(i,:);
%             last_pbest(i,:) = pbest(i,:);
