function [gbest,gbestval,hist_v,hist_p,FES]= EPSO_func(fhd,Dimension,num_particle,max_iteration,VRmin,VRmax,varargin)
...
for k = 1:me-1
        gbest_pos_temp=repmat(gbest,ps,1);
...
if k<=(0.9*me)
      ...
      for i=num_g1+1:num_g1+num_g2
            ...
            if probability>= rk(1) && probability < rk(2)
                % PSO  
                strategy_k = 1;
                delta(i,:)=c2_1(k).*rand(1,D).*(pbest(i,:)-pos(i,:))+c2_2(k).*rand(1,D).*(gbest_pos_temp(i,:)-pos(i,:));
            ...
            elseif probability>= rk(2) && probability < rk(3)   
                ...
                delta(i,:)=fii(1).*rand(1,D).*(pbest(i,:)-pos(i,:))+fii(2).*rand(1,D).*(gbest_pos_temp(i,:)-pos(i,:))+fii(3).*rand(1,D).*(Pnd(i,:)-pos(i,:));
                vel(i,:)=w2(k).*vel(i,:)+delta(i,:); 
            ...
            elseif probability>= rk(3) && probability < rk(4)   
                % HPSO
                strategy_k = 3;
                vel(i,:)=(c3_1(k).*rand(1,D).*(pbest(i,:)-pos(i,:)))+(c3_2(k).*rand(1,D).*(gbest_pos_temp(i,:)-pos(i,:)));
                for d=1:D
                    if vel(i,d)==0
                        if (rand(1)<0.5)
...
%              
           elseif probability>= rk(5) && probability < rk(6)
                    %CLPSO
                    strategy_k = 5;                          
                    delta(i,:)=(c4_1(k).*rand(1,D).*(fri_best_pos(i,:)-pos(i,:)))+(c4_2(k).*rand(1,D).*(gbest_pos_temp(i,:)-pos(i,:)));
                    vel(i,:)=w2(k)*vel(i,:)+delta(i,:);
