function [gbest,gbestval,hist_v,hist_p,FES]= PID_EPSO_func(fhd,Dimension,num_particle,max_iteration,VRmin,VRmax,varargin)
...
last_delta = zeros(ps,D);
last_delta1 = zeros(ps,D);
last_delta2 = zeros(ps,D);
last_delta3 = zeros(ps,D);
last_delta4 = zeros(ps,D);
...
if k<=(0.9*me)
      ...
      for i=1:num_g1 
            ...
            der(i,:)= alpha*der(i,:) + (delta(i,:)-last_delta(i,:));
            last_delta(i,:) = delta(i,:);
            pos(i,:)=pos(i,:)+ki*vel(i,:)+kp*der(i,:); 
      ...
      for i=num_g1+1:num_g1+num_g2
            ...
            if probability>= rk(1) && probability < rk(2)
                ...
                der(i,:)=alpha*der(i,:)+(delta(i,:)-last_delta1(i,:));
                last_delta1(i,:) = delta(i,:);
            ...
            elseif probability>= rk(2) && probability < rk(3)   
                ...
                vel(i,:)=w2(k).*vel(i,:)+delta(i,:); 
                der(i,:)=alpha*der(i,:)+(delta(i,:)-last_delta2(i,:));
                last_delta2(i,:) = delta(i,:);
            ...
            elseif probability>= rk(4) && probability < rk(5)
                    ...
                    der(i,:)=alpha*der(i,:)+(delta(i,:)-last_delta3(i,:));
                    last_delta3(i,:) = delta(i,:);
%              
           elseif probability>= rk(5) && probability < rk(6)
                    ...
                    vel(i,:)=w2(k)*vel(i,:)+delta(i,:);
                    der(i,:)=alpha*der(i,:)+(delta(i,:)-last_delta4(i,:));
                    last_delta4(i,:) = delta(i,:);
                     if obj_func_slope(i)>5
                       fri_best(i,:)=i*ones(1,D);
                       friend1=(ceil(ps*rand(1,D)));
                       friend2=(ceil(ps*rand(1,D)));
