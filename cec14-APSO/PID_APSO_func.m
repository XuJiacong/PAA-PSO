function [gbest,gbestval,hist_v,hist_p,fitcount]= PID_APSO_func(fhd,Dimension,Particle_Number,Max_Gen,VRmin,VRmax,varargin)
...
for i=2:Max_Gen 
    ...
    vv = c1.*rand(ps,D).*(pbest - pos) + c2.*rand(ps,D).*(repmat(gbest,ps,1) - pos);
    vel = w*vel + vv;
    % Update D component
    der = alpha.*der + (vv - 2.*vv_last + vv_last2);
    vv_last2 = vv_last; 

    % Update P component
    pro = alpha.*pro + (vv - vv_last);
    vv_last = vv;
    
    vel = (vel>Vmax).*Vmax+(vel<=Vmax).*vel;
    vel = (vel<Vmin).*Vmin+(vel>=Vmin).*vel;
    % D clamping
    der = (der>Vmax) .* Vmax + (der<=Vmax) .* der;
    der = (der<Vmin) .* Vmin + (der>=Vmin) .* der;
    % P clamping
    pro = (pro>Vmax) .* Vmax + (pro<=Vmax) .* pro;
    pro = (pro<Vmin) .* Vmin + (pro>=Vmin) .* pro;
